...
class ForegroundService : Service() {
    ...
    private var aimTime:Long = 60000L
    ...
    private fun processCommand(intent: Intent?) {
        when (intent?.extras?.getString(COMMAND_ID) ?: INVALID) {
            COMMAND_START -> {
                ...
                aimTime = intent.extras?.getLong(AIM_TIMER_TIME_MS) ?: return
        ...
        job = GlobalScope.launch(Dispatchers.Main) {
            while (true) {
                notificationManager?.notify(
                    NOTIFICATION_ID,
                    getNotification(
                        (aimTime-(System.currentTimeMillis() - startTime)).displayTime()
                    )
                )
                delay(INTERVAL)
            }
    ...
    private fun getPendingIntent(): PendingIntent? {
        ...
        resultIntent.putExtra("reconnect",1)
